...
* **📖 시 낭송 시 문단(연) 실시간 연한 노랑색 하이라이트 기능 추가**:
  * 시 낭송(TTS) 재생 시 현재 낭독 중인 **시 제목, 각 문단(연, Stanza), 그리고 시인의 노트**를 **연한 노랑색(`Soft Pastel Yellow`, `#fef9c3`)**과 은은한 황금빛 포인트 바로 실시간 강조 표시합니다.
  * 낭독 진행 흐름에 맞추어 현재 문단으로 시선이 편안하게 머물 수 있도록 **부드러운 자동 스크롤(Auto Smooth Scrolling)**이 연동됩니다.
  * 낭송이 끝나거나 멈출 경우 하이라이트가 부드럽게 초기화되어 본래의 깔끔한 한지 서정 디자인으로 복원됩니다.
* **🏷️ 전역 버전 표기 체계 `Ver-2` 갱신**: 웹 UI 뱃지, 백엔드 API, 콘솔 및 문서화 일괄 반영.

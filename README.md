...
### 3. ✨ 시 분위기 맞춤 Google TTS 감성 자동 튜닝 (신규)
* **시 정서별 음향 파라미터 자동 산출**: 선택한 시의 분위기에 맞춰 Google TTS의 세부 옵션 값이 실시간으로 자동 조절됩니다:
  * **김소월 풍 (그리움)**: Pitch `0.92x` (-6%), Rate `0.78x`, 연간 여운 `1.4초` (아련하고 애절한 한의 호흡)
  * **윤동주 풍 (사색)**: Pitch `1.00x` (0%), Rate `0.79x`, 연간 여운 `1.2초` (절제된 고요와 순결한 사색)
  * **박목월 풍 (자연)**: Pitch `1.04x` (+4%), Rate `0.83x`, 연간 여운 `1.0초` (청명하고 소박한 자연의 숨결)
  * **정호승 풍 (위로)**: Pitch `0.94x` (-4%), Rate `0.75x`, 연간 여운 `1.5초` (나직하고 포근한 온기의 속삭임)
  * **나태주 풍 (설렘)**: Pitch `1.07x` (+7%), Rate `0.87x`, 연간 여운 `0.9초` (미소 띤 첫사랑의 다정한 설렘)
* **자연스러운 시적 호흡 지연(Pause) 엔진**: 기계음처럼 끊김 없이 한 번에 읽지 않고, 연(Stanza)과 행(Line) 사이에 시적 여운을 주는 지능형 호흡 지연을 적용하여 서정성을 극대화했습니다.
